...
class MarineBot(BotAI):
    ...
    async def on_step(self, iteration: int):
        ...
        colossus_units = self.units(UnitTypeId.COLOSSUS)
        self.colossus = colossus_units.first if colossus_units else None
        self.stalkers = self.units(UnitTypeId.STALKER)
        self.zealots = self.units(UnitTypeId.ZEALOT)

        enemy_colossus_units = self.enemy_units(UnitTypeId.COLOSSUS)
        enemy_colossus = enemy_colossus_units.first if enemy_colossus_units else None
        ...
        if enemy_colossus and self.colossus and self.colossus.distance_to(enemy_colossus) <= 7:
            self.colossus.attack(enemy_colossus)
            if enemy_zealots:
                for zealot in self.zealots:
                    closest_enemy_zealot = enemy_zealots.closest_to(zealot)
                    if zealot.distance_to(closest_enemy_zealot) <= 0.1:
                        zealot.attack(closest_enemy_zealot)
            if enemy_stalkers:
        ...
        # Tactic 2: Stalker Kiting
            ...
            for stalker in self.stalkers:
                ...
                if stalker.distance_to(closest_enemy_zealot) <= 6:
                    stalker.move(stalker.position.towards(self.colossus, 4) if self.colossus else stalker.position.towards(Point2((9, 16)), 4))
                    stalker.attack(closest_enemy_zealot)
            if self.colossus and enemy_zealots:
        ...
        # Tactic 3: Zealot Rush
            ...
            if self.colossus and enemy_stalkers:
